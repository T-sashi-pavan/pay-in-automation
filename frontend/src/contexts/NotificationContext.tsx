...
interface NotificationContextValue {
  notify: (message: string, type: NotificationType, duration?: number, action?: NotificationAction) => void;
}
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    if (notification && notification.duration !== 0) {
      const ms = notification.duration !== undefined ? notification.duration : 5000;
      const timer = setTimeout(() => setNotification(null), ms);
      return () => clearTimeout(timer);
    }
  }, [notification]);

  const notify = (message: string, type: NotificationType, duration?: number, action?: NotificationAction) => {
    setNotification({ message, type, duration, action });
  };
    ...
    <NotificationContext.Provider value={{ notify }}>
      ...
      {notification && (
        <div className="fixed bottom-6 right-6 z-[100] animate-in fade-in slide-in-from-bottom-2 duration-300">
          <div className={`flex items-center gap-3 px-4 py-3.5 rounded-xl border shadow-lg backdrop-blur-md min-w-[320px] max-w-md ${
            notification.type === 'success'
              ? 'bg-white/95 dark:bg-slate-900/95 border-emerald-200 dark:border-emerald-500/30'
              : notification.type === 'info'
              ? 'bg-white/95 dark:bg-slate-900/95 border-blue-200 dark:border-blue-500/30'
